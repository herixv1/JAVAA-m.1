...
public class VMascotasA {
	// declaracion del arreglo de objetos
	private Mascota mascotas[];
	// tope nos indica el ultimo valor que se almacena en el vector y representa la
	// longitud logica del vector o arreglo
	private int tope;

	// en el constructor creamos el vector o arreglo y inicializamos tope a -1 que
	// representa que esta vacio.
	public VMascotasA(int tam) {
	...
	public VMascotasA buscarObjetoVector(Mascota o) {
		VMascotasA encontradas = new VMascotasA(this.tope);
		for (int i = 0; i <= tope; i++) {
			if (mascotas[i].compareTo(o) == 1)
	...
	public void ordenarDueno() {
		Mascota temp;
		for (int i = 0; i < this.tope; i++) {
			for (int j = i; j <= this.tope; j++) {
				if (mascotas[i].getDuenio().getNombre().compareToIgnoreCase(mascotas[j].getDuenio().getNombre()) > 0) {
					temp = mascotas[i];
					mascotas[i] = mascotas[j];
					mascotas[j] = temp;
				}
	...
	public void ordenarDescendente() {
		...
		for (int i = 0; i < this.tope; i++) {
			for (int j = i; j <= this.tope; j++) {
				// Evaluamos con < 0 para que sea descendente
				if (mascotas[i].getNombre().compareToIgnoreCase(mascotas[j].getNombre()) < 0) {
					temp = mascotas[i];
					mascotas[i] = mascotas[j];
					mascotas[j] = temp;
